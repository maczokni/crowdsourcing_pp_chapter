...
#Employment <- read.csv("covariates_model/Unemployment_2015.csv")
#Income <- read.csv("covariates_model/Income_2015.csv")
...
# merge all independent variables in a dataset

#exp_variables <- list(Employment, Income) %>% 
#  reduce(left_join, by = "TRACTCE10") %>%
#  select(TRACTCE10, Percent_Percent_Unemployed__Civ, Average_Per_capita_Inc)

exp_variables <- list(Migration, Whites, Blacks, Occ_houses, rent1000, Income) %>% 
  reduce(left_join, by = "GEOID") %>%
  select(GEOID, pMoved_e, pNHWhite_e, pNHBlack_e, pOccHU_e, pGrent1000P_e, pHHIncLt10k_e)

#Exp_variables <- merge(Migration, Whites, by = "GEOID")
...
exp_variables$GEOID10 <- as.factor(exp_variables$GEOID10)

class(atl_pp_wins$TRACT)
atl_pp_wins$TRACT <- as.numeric(atl_pp_wins$TRACT)



# merge dependent and independent variables
atl_pp_covs <- atl_pp_wins %>%
  left_join(exp_variables, by = c("TRACT" = "TRACTCE10"))
#Atlanta_map3 <- merge(Atlanta_map2, Exp_variables, by = "GEOID", all.x = TRUE) 
...
#Atlanta_map3$Safety_s <- (Atlanta_map3$Safety-mean(Atlanta_map3$Safety))/sd(Atlanta_map3$Safety)
...
label <- c(
  `(Intercept)`   = "Intercept",
  pMoved_e_s      = "% Moved in last year",
  pNHWhite_e_s    = "% White",
  pNHBlack_e_s    = "% African American",
  pOccHU_e_s      = "% Occupied housing units",
  pGrent1000P_e_s = "% Rent $1,000 or more"
)

tab_model(model, model.lag, model.error, pred.labels = label)
