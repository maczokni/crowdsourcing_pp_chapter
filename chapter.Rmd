...
![Figure 1: Place Pulse website](https://github.com/maczokni/crowdsourcing_pp_chapter/blob/master/figures/Place_Pulse.jpg?raw=true)
...
For instance, we can see which cities had a largest number of votes amongst images in the left part of the pairwise comparison:
...
Safety was the most commonly assessed variable, with 509,961 votes in total. Before start analysing the data, we can also examine if participants were more inclined to vote for images in the left or right part of the platform.

```{r}
top_n(count(pp_data$choice), 3)

```

The frequency of votes for left and right options is very similar, which shows that the position of the image in the comparison does not have a direct affect on participants' decision.
...
First, we want to know the number of comparisons in which both images are from Atlanta.
```{r}
...
In total, 678 pairwise-comparisons included in our dataset are based on two images from the city of Atlanta, whereas 36,536 compare one image from Atlanta with a picture from another city. In order to make maximal usage of crowdsourced data, we will 

We will duplicate these comparisons in a new dataset, which will be later merged with the original 
