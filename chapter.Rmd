...
Researchers may also be interested in analyzing the environmental characteristics of those places that are the safest or the least safe according to Place Pulse participants. We show below some lines of code that will print the coordinates of places rated as the safest and the least safe. To illustrate this, we only consider those places that have been evaluated at least 20 times, to increase the validity of these rates, but one may decide to reduce or increase this threshold value according to different criteria. For example, if we were interested in obtaining the coordinates of the five places rated as the safest we would need to change the last line of code by `head(5)`, or `tail(5)` to obtain the coordinates of the least safe places.
...
This allows criminologists to study the characteristics of those places rated by participants are more or less safe. If we use Google Street View [[https://www.google.com/maps](https://www.google.com/maps)] to observe the places assessed by Place Pulse as the least safe and safest amongst those rated by at least 20 participants, we can easily see that the least safe place is characterized by very dense vegetation (which may be perceived to offer concealment for possible criminals), lack of exit routes to escape from a potential threat, and an abandoned house with signs of physical disorder (see @fisher1992)
