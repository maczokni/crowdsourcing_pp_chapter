...
pp_data %>%
  group_by(study_question) %>% # categories based on the questions
  summarize(Count = n()) # count number of units in each category
...
pp_data %>%
  group_by(choice) %>% # categories based on vote
  summarize(Count = n()) %>% # count number of units in each category
  top_n(3) # print 3 most frequent categories
...
We can use the function `filter()` to create a new dataframe that includes those pairwise comparisons in which either the image of the right or the image of the left, or both, are from Atlanta.
